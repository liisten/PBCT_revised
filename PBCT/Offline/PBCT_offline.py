...
def PBCT_log(csv_name,L,U,random_index,repeated_num,coef_file):
    ...
    for i in range(y_test.shape[0]):
        ...
        tmp_pre = np.matmul(alpha.T,X_train_unlabled.iloc[i].to_numpy())*std_labled_y+mean_labled_y
    ...
    for i in range(y_test.shape[0]):
        tmp_y = y_test.iloc[i].to_numpy()*std_labled_y+mean_labled_y
        tmp_pre = Reg.predict([ZU_all.iloc[i].to_numpy()])[0]*std_labled_y+mean_labled_y
        tmp_error = (tmp_y.to_numpy()[0] - tmp_pre.to_numpy()[0])
        tmp_percent_err =abs(tmp_error/tmp_y.to_numpy()[0])*100
        tmp_error_square = tmp_error * tmp_error
...
if __name__=="__main__":
    ...
    for sweep_num in range(7,16):
        tmp_dict ={}
        all_list  = []
        for i in range(repeated_num):
            print('the ',i+1,'-th result')
            PBCT_RMSE,  PBCT_beta_RMSE,LS_beta_RMSE,LS_lasso_RMSE,LS_elasnet_RMSE = PBCT_log(csv_name,sweep_num,unlabeled_num,random_index[i],i,coef_file)
